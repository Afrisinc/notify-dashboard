...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: 'hsl(224,18%,10%)', border: `1px solid hsl(224,14%,18%)`, borderRadius: 8, padding: '10px 14px' }}>
      ...
      {payload.map(p => (
        <p key={p.name} style={{ fontSize: 12, color: p.color, marginBottom: 2 }}>{p.name}: <strong>{typeof p.value === 'number' && p.value > 100 ? p.value.toLocaleString() : p.value}</strong></p>
      ))}
    </div>
  )
}
...
function Card({ children, title, subtitle, style }) {
  return (
    <div style={{ background: 'hsl(224,18%,8%)', border: `1px solid hsl(224,14%,14%)`, borderRadius: 12, padding: '20px 24px', ...style }}>
      {(title || subtitle) && (
        <div style={{ marginBottom: 20 }}>
          {title && <p style={{ fontWeight: 600, fontSize: 15, color: 'hsl(210,20%,95%)', marginBottom: 2 }}>{title}</p>}
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: 'hsl(210,20%,95%)', letterSpacing: '-0.02em', marginBottom: 4 }}>Analytics</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 24 }}>
        {[
          ...
          <div key={k.label} style={{ background: 'hsl(224,18%,8%)', border: `1px solid hsl(224,14%,14%)`, borderRadius: 12, padding: '20px 22px' }}>
            <p style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.02em', color: 'hsl(210,20%,95%)', marginBottom: 4 }}>{k.value}</p>
            ...
            {[
              ...
              <div key={r.label}>
                ...
                <div style={{ height: 6, borderRadius: 3, background: 'hsl(224,14%,14%)' }}>
                  <div style={{ height: '100%', borderRadius: 3, background: r.color, width: `${Math.min(r.pct, 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <Card title="Email Engagement" subtitle="Open rate, click rate & unsubscribes (%) this week">
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={engagementData} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
              ...
              <Line type="monotone" dataKey="unsubscribes" stroke={C.destructive} strokeWidth={2} dot={false} name="Unsubs %" />
            </LineChart>
          </ResponsiveContainer>
        </Card>
                ...
                <div key={c.name}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontFamily: 'JetBrains Mono', fontSize: 11, color: 'hsl(215,15%,45%)', minWidth: 16 }}>#{i + 1}</span>
                      <span style={{ fontSize: 13, fontWeight: 500, color: 'hsl(210,20%,85%)' }}>{c.name}</span>
                      <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 6px', borderRadius: 9999, background: 'rgba(2,147,228,0.1)', color: '#36A9EA' }}>{c.plan}</span>
                    </div>
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'hsl(215,15%,60%)' }}>{(c.sent / 1000).toFixed(0)}K</span>
                  </div>
                  <div style={{ height: 5, borderRadius: 3, background: 'hsl(224,14%,13%)' }}>
                    <div style={{ height: '100%', borderRadius: 3, background: `hsl(${187 + i * 15},70%,48%)`, width: `${(c.sent / max) * 100}%`, transition: 'width 0.5s ease' }} />
                  </div>
                </div>
              )
            })}
